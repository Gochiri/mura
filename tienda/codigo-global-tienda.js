/* ============================================================
   ...
   ============================================================
   Va en: Sitio "Mura" → Settings → Tracking Code → BODY
   (en el DOM aparece como <div id="gb-track-hl-custom-code">)

   Tiene cinco partes, en este orden y por este motivo:

     1. El sistema de diseño `.mura-*`  → TODAS las páginas
     2. /cart redirige a /carrito       → solo /cart
     3. CSS que pisa la tienda de GHL   → solo páginas de tienda
     4. Cabecera en las páginas de tienda
     5. Contador del carrito            → TODAS las páginas

   Las partes 3 y 4 van detrás de un guardia de rutas. Las partes
   1 y 5 NO: son de todo el sitio.
...
(function () {
  ...
  st1.setAttribute('data-mura', 'sistema');
  st1.textContent = sistema;
  ...
  function pintarContador() {
    ...
    document.querySelectorAll('.mura-cart-n').forEach(function (e) { e.textContent = n; });
    document.querySelectorAll('.mura-cart-w').forEach(function (e) { e.style.display = n > 0 ? 'inline' : 'none'; });
  }

  /* ============================================================
     2 · /cart se retira: el carrito bueno es /carrito
     ============================================================
     ...
     Va antes que nada y con `replace` en vez de `href`: así no
  ...
  /* ============================================================
     3 y 4 · Solo en las páginas que renderiza la tienda de GHL
     ============================================================
     A partir de aquí se pisa el CSS del elemento de tienda, que
     no es nuestro. En el resto del sitio no hay nada que pisar. */

  var RUTAS_TIENDA = ['/products-list', '/product-details', '/checkout', '/thank-you'];
  if (RUTAS_TIENDA.indexOf(path) === -1) {
    pintarContador();
    return;
  }
  ...
  st2.setAttribute('data-mura', 'tienda');
  st2.textContent = tienda;
